...
const OrderDetailsCustom = (props) => {
  const {
    detailsForm,
    detailsLoading,
    dashboardData,
  ...
  useEffect(() => {
    if (batteryCondition && transportFile && packagingType) {
  ...
  const handleDropdownValues = (value) => {
    let newValues = [];
  ...
  const handleResponse = (result, formik) => {
    if (result?.success) {
      cartItemLength();
      setButtonLoading(false);
      close();
  ...
  const apiCall = async (formik) => {
    if (
      (batteryCondition && transportFile && packagingType) ||
      (batteryCondition &&
        transportFile &&
        batteryCondition === _542f61af_478c_48df_af9e_fbcb0337c155)
    ) {
      ...
      handleResponse(result, formik);
    }
  };

  const serviceTypePickup = async (formik) => {
    if (!batteryCondition) {
    ...
    if (!transportFile && Category === "Hochvoltspeicher Auto") {
...
export default OrderDetailsCustom;
